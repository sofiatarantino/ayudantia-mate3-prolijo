...
\begin{solution}
    ...
    Como $f,g \in \mathcal{C}^1( \mathbb{R}^2)$, el teorema de los multiplicadores de Lagrange nos dice que si $f \big|_A$ tiene un extremo local en un punto $\mathbf{x}$, entonces necesariamente los vectores $\nabla f(\mathbf{x})$ y $\nabla g(\mathbf{x})$ son paralelos si $\nabla g(\mathbf{x}) \neq 0$. Además, como $f$ es continua sobre el conjunto $A$ que es cerrado y acotado, por el teorema de Weierstrass, sabemos que $f$ alcanza máximo y mínimo en $A$. Luego planteamos el siguiente sistema de ecuaciones:
\[
    \begin{aligned}
        \nabla f(x,y) &= \lambda \nabla g(x,y), \quad \lambda \in \mathbb{R}, \; (x,y) \in A \\[6pt]
        &\iff \begin{cases}
            2x = 2\lambda(x-1) \\
            2y = 2\lambda(y-1) \\
            (x-1)^2 + (y-1)^2 = 4
