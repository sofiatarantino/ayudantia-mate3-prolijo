...
Observemos que $\mathbf{T}$ es precisamente una \emph{transformaci\'on lineal} en el sentido de la Definici\'on \ref{def:TL}: dados $(u_1,v_1),(u_2,v_2)\in\Rn{2}$ y $\alpha,\beta\in\R$, se cumple
\begin{equation*}
\mathbf{T}\big(\alpha(u_1,v_1)+\beta(u_2,v_2)\big)=\alpha\,\mathbf{T}(u_1,v_1)+\beta\,\mathbf{T}(u_2,v_2),
\end{equation*}
es decir, $\mathbf{T}$ satisface la condici\'on de linealidad (\ref{tl}).
...
es decir, el jacobiano de una transformaci\'on lineal es constante, no depende del punto $(u,v)$ donde se lo eval\'ue, y coincide con el determinante de la matriz que define a $T$. Esto es consecuencia directa de que $T$ es lineal: sus derivadas parciales son constantes, por lo que tambi\'en lo es $\boldsymbol{D}_\mathbf{T}$ y, en consecuencia, $J_\mathbf{T}$.

Otra propiedad importante de este cambio de coordenadas es que, al ser $T$ lineal, las rectas de la regi\'on $D^*$ se transforman en rectas de la regi\'on $D$: las figuras no se curvan, s\'olo se rotan, trasladan o estiran. En particular, $T$ manda paralelogramos en paralelogramos.
...
\begin{figure}[htbp]
    ...
    \input{tikz/Integrales dobles y triples/Lineales.tex} 
