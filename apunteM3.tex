\documentclass{report}
\input{settings.tex}
\usepackage{float}

\begin{document}
%===================================================
%                     CARATULA
%===================================================
    \begin{center}

        \thispagestyle{empty}  % Saca la numeracion a la primer pagina

        \includegraphics[scale=0.2]{figs/logoitba2.png}
        \medskip
        
        \textbf{\large{INSTITUTO  TECNOL\'OGICO DE BUENOS AIRES}\normalsize}
        
        \smallskip
        
        \textbf{\large  Departamento de Ciencias Exactas y Naturales}
        
        \smallskip
        \textbf{\large \'Area  Matem\'atica}
      
        \vspace{2 cm}
        
        \textbf{\Large \noindent \textbf{An\'alisis matem\'atico III.}}
        
        \textbf{\Large \noindent \textbf{Apunte Te\'orico.}}
             
        \vspace{1.5cm}
        
       
    \end{center}
    
    \newpage

%===================================================
%                   INTRODUCCION
%===================================================
%    \setcounter{page}{1}
 %   \begin{center}
  %      \textbf{ \noindent \textbf{Parciales Resueltos}}
  %  \end{center}
  %  \vspace{3cm}
      
   % (Pequena explicacion).  Este documento presentaremos distintos ejercicios de parcial....
   % \newpage

%===================================================
%                      INDICE
%===================================================
    \tableofcontents
        \pagenumbering{arabic}
        \setcounter{page}{1}
    \vfill
  %  \textcolor{red}{algun tipo de nota explicando detalles de notacion, por ej.} Los símbolos para los vectores serán marcados en negrita minúscula, $\mathbf{v},\;\mathbf{x},\;\mathbf{n},\;\boldsymbol{\sigma}$ etc., y mayúscula los campos vectoriales, $\mathbf{F},\;\mathbf{G},\;\boldsymbol{\Sigma}$, etc. Las matrices estarán en negritas e italicas, $\boldsymbol{D},\;\boldsymbol{H}$, etc.

%===================================================
%                CONTENIDO TEORICO
%===================================================
 \chapter{Recuerdo de An\'alisis en una Variable y Algebra}


\section{Recuerdo de Análisis Matemático 1}
\input{capitulos/recuerdo_Mate1.tex}
          
      
        
\section{Recuerdo de Álgebra}
\input{capitulos/recuerdo_Algebra.tex}
           
     


\chapter{Nociones de Topolog\'ia en $\mathbb{R}^{n}$}
\section{Normas y Distancias en $\mathbb{R}^{n}$}
\input{capitulos/topo_norma_distancia.tex}

\section{Bolas y Entornos en  $\mathbb{R}^{n}$}
\input{capitulos/topo_bola_entorno.tex}


\section{Punto interior}
\input{capitulos/topo_punto_interior.tex}


\section{Conjuntos cerrados y abiertos}
\input{capitulos/topo_abierto_cerrado.tex}


\section{Frontera}
\input{capitulos/topo_frontera.tex}


\section{Punto de acumulación}
\input{capitulos/topo_punto_acumu.tex}


\section{Punto aislado}
\input{capitulos/topo_punto_aisla.tex}

\section{Conjuntos Acotado}
\input{capitulos/topo_conj_acot.tex}


\section{Conjunto Conexo}
\input{capitulos/topo_conj_conexo.tex}



\section{Conjunto Arcoconexo}
\input{capitulos/topo_conj_arcoconexo.tex}


\section{Conjuntos simplemente conexos}
\input{capitulos/topologia.tex}


\section{Conjunto  Convexo}
\input{capitulos/topo_conj_convexo.tex}







\chapter{Campos Escalares y Vectoriales.}
         
\section{Definici\'on de Campos Vectoriales y Escalares}
\input{capitulos/campos.tex}
    \section{Definici\'on de Trayectorias y Parametrizaciones}
\input{capitulos/Trayectorias_Paramatrizaciones.tex}


  


\section{Gráficas}
\input{capitulos/Grafica.tex}
         



            
\section{Conjuntos de nivel}
\input{capitulos/conjunto_de_nivel.tex}
       
  
  
         
 \section{L\'imite y continuidad de Campos Escalares y Vectoriales}
 \input{capitulos/limites.tex}
   \input{capitulos/continuidad.tex}    

  
          
\chapter{Diferenciabilidad.} 
          \section{Derivadas Direccionales}
       \input{capitulos/derivabilidad.tex}
            
 
\section{Diferenciabilidad}
\input{capitulos/diferenciabilidad.tex}
 
\section{Regla de la Cadena}
 \input{capitulos/cadena.tex}



\section{Funci\'on impl\'icita e inversa}    
   \input{capitulos/inversa_implicita.tex}
      

 \chapter{Polinomio de Taylor y Extremos.}

  \section{Polinomio de Taylor} 
   \input{capitulos/Polinomio_de_Taylor.tex}
   
  \section{Extremos de campos escalares} 
   \input{capitulos/extremos.tex}

 \chapter{Operadores.}
  \section{Operadores}    
  \input{capitulos/operadores.tex}
   
  \chapter{Integrales de Trayectoria y de  L\'inea.}
                       \section{Integrales de Taryectoria y de L\'inea de campos escalares}
        \subsection{Longitud de curva} 
             \input{capitulos/longitud_de_curva.tex}
              \subsection{Integral de trayectoria de campos escalares}
                 \input{capitulos/integral_trayectoria_campos_escalares.tex}
               \subsection{Integral de línea para campos escalares}
                 \input{capitulos/integral_linea_campos_escalares.tex}
                \subsection{Propiedades del Álgebra}
             \input{capitulos/algebra_integral_linea_esc.tex}
  
\section{Integrales de Trayectoria y de L\'inea de campos vectoriales}
         \subsection{Integral de trayectoria de campos vectoriales}
                 \input{capitulos/integral_trayectoria_campos_vectoriales.tex}
        \subsection{Integral de línea para campos vectoriales}
             \input{capitulos/integral_linea_campos_vectoriales.tex}
   \subsection{Propiedades del Álgebra}
             \input{capitulos/algebra_integral_linea_vec.tex}
         

      
  \section{Campos Conservativos}
  \subsection{Teorema fundamental de las integrales de línea}
  \input{capitulos/teorema_fundamental_integrales_de_linea.tex}
  \subsection{Función Potencial}
  \input{capitulos/campo_vectorial_conservativo.tex}
   \subsection{Independencia del camino}
   \input{capitulos/independencia_del_camino.tex}
   \subsection{Teorema: Independencia y Potencial}
   \input{capitulos/funcion_potencial_independencia_del_camino.tex}
   \subsection{Equivalencias de Campos Conservativos}
 \input{capitulos/equivalencias_conservativos.tex}


  
    \chapter{Integrales dobles y triples de  campos escalares.}
     \section{Integrales Dobles} 
        \subsection{Integrales Dobles sobre Rectángulos}
         \input{capitulos/integrales_dobles_Rectángulos.tex}
           \subsection{Integrales dobles sobre conjuntos generales}
          \input{capitulos/integrales_dobles_generales.tex}
           \subsection{Integrales dobles sobre conjuntos elementales}
           \input{capitulos/integrales_dobles_elementales.tex}
      \subsection{Teorema de Fubbini}
      \input{capitulos/integrales_dobles_fubbini.tex}
          \subsection{Teorema de Valor Medio Integral}
            \input{capitulos/TVM_integral_r2.tex}
         \subsection{Teorema de Cambio de Variables en $\mathbb{R}^{2}$}
          \input{capitulos/cambio_de_variables_r2.tex}
           \subsection{Coordenadas Polares}
            \input{capitulos/polares.tex}
            \subsection{Coordenadas Lineales}
            \input{capitulos/coordenadas_lineales.tex}

       
  

     
       
     \section{Integrales Triples}
          \subsection{Integrales Triples sobre Paralelepipedos}
          \input{capitulos/integrales_triples_Paralelepipedos.tex}

           \subsection{Integrales Triples sobre conjuntos generales}
                 \input{capitulos/integrales_triples_generales.tex}


           \subsection{Integrales Triples sobre Conjuntos Elementales}
                 \input{capitulos/integrales_triples_elementales.tex}

           \subsection{Teorema de Fubini}
               \input{capitulos/integrales_triples_fubbini.tex}


           \subsection{Teorema del Valor Medio Integral}
                   \input{capitulos/TVM_integral_r3.tex}
              
           \subsection{Teorema de Cambio de Variables en $\mathbb{R}^{3}$}
                 \input{capitulos/cambio_de_variables_r3.tex}

                 \subsection{Coordenadas Esfericas}
                 \input{capitulos/esfericas.tex}

               \subsection{Coordenadas Cilindricas}
                 \input{capitulos/cilindricas.tex}
          
          \subsection{Ejemplos}
         \input{ejercicios/ejercicio16.tex}
         \input{ejercicios/ejercicio27.tex}
          \input{ejercicios/ejercicio28.tex}
          \input{ejercicios/ejercicio30.tex}
           \input{ejercicios/ejercicio21.tex} 
   
  

  \chapter{Superficies en $\mathbb{R}^{3}$ } 
      \section{Superficies parmetrizadas.}
         \input{capitulos/integrales_superficie.tex}
  


   

  \chapter{Teoremas Integrales }       
       \section{Teorema de Green}
       \subsection{Teorema de Green}
            \input{capitulos/green.tex}
        \subsection{Cálculo de Área}
            \input{capitulos/area_green.tex}
        \subsection{Extensión del Teorema de Green}
            \input{capitulos/extension_green.tex}
        \subsection{Teorema de la Divergencia en el Plano}
            \input{capitulos/divergencia_enelplano.tex}
     \section{Teorema de Stokes}
        \subsection{Teorema de Stokes}
            \input{capitulos/stokes.tex}
        \section{Teorema de Gauss}
        \subsection{Teorema de Gauss}
            \input{capitulos/gauss.tex}








%===================================================
%               EJERCICIOS RESUELTOS
%===================================================
    \chapter{Ejercicios resueltos}
        \section{Fecha 29 de septiembre de 2023}
            \input{capitulos/1p_29_09_2023}

    
    
        \newpage
        \section{Fecha 25 de abril de 2023}
            \input{capitulos/1p_25_04_2023}





            \newpage
        \section{Fecha 23 de septiembre de 2023}
            \input{capitulos/1P_23_09_2023}


            \newpage
        \section{Fecha 18 de abril de 2024}
            \input{capitulos/1P_18_04_2024}
            


        \newpage
        \section{Fecha Recuperatorio 2cuatri 2022}
            \input{capitulos/1p_recu_2Q_2022}



        \newpage
        \section{Fecha 25 de junio de 2023}
            \input{capitulos/2p_25_06_2023}



        \newpage
        \section{Fecha recuperatorio 29 de junio de 2023}
            \input{capitulos/2p_recu_29_06_2023}



        \newpage
        \section{Fecha 18 de noviembre de 2023}
            \input{capitulos/2p_18_11_2023}



        \newpage
        \section{Fecha 11 de junio de 2024}
            \input{capitulos/2p_11_06_2024}
     
    
      %  \newpage
        %\section{Fecha 28 de septiembre de 2024}  Hay algun problrma de compilacion
           % \input{capitulos/1P_28_09_2024}
          
           
            
       % \newpage
        %\section{Fecha 11 de noviembre de 2024} Hay algun problrma de compilacion
           % \input{capitulos/2P_11_11_2024}
          
                  \newpage
        \section{Fecha recuperatorio extra}
            \input{capitulos/2doRecupExtra}
\end{document}

